		...
		while(fighting)
		{
			...
			switch(battling)	//Switches what the player wants to do
			{
				case 1:		//The player wants to fight
							//Chooses who attacks first at random
							int firstFighter = (int) (Math.random() * 2); 
							
							if(firstFighter == 1) //Player gets to attack first
							{
							...
							else	//Opponent gets to attack first
							{
								trainerBattle(opponent, player); //opponent attacks
								player.displayCurrentPokemon(opponent.getCurrentPokemon());
								...
								if(player.getCurrentPokemon().getHP() == 0)
								{
									...
									{
										System.out.println("All of your pokemon has fainted.");
										System.out.println(opponent.getName() + " has defeated you! You lost 25 hp for losing!");
										opponent.winSpeech();
										player.lossSpeech();
										player.loseHP(20);
										fighting = false;
								...
								if(fighting)
								{
									...
									{
										//Calculates remaining hp = current hp/Max hp
										double remainingHP = (opponent.getCurrentPokemon().getHP())/
												opponent.getCurrentPokemon().getMaxHP();
		...
		while(fighting)
		{
			...
			else if(choice == 2)
			{
				...
				if(chance > 0.50)
				{
					...
					{
						System.out.println("less than quarter");
